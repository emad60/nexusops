...
### Community 5 - "deps.py"
Cohesion: 0.07
Nodes (48): asyncio, FastAPI dependencies: database session, authentication context, RBAC gate.…, Agent ingest endpoints: enrollment handshake and periodic heartbeats.…, AppError, Forbidden, Exception, Error taxonomy and the single place where API error envelopes are shaped. Every…, Base class for expected, client-facing errors. (+40 more)

### Community 6 - "resolve_client_ip"
Cohesion: 0.16
Nodes (23): _is_trusted(), _parse_networks(), Request, Client-IP resolution behind chained reverse proxies. The API never takes TCP…, Parse a comma-separated CIDR list; malformed entries are skipped., Real client address for *request* per the trust model above., resolve_client_ip(), make_request() (+15 more)
...
Cohesion: 0.21
Nodes (9): LogLevel, _commit_short(), Deployment runner port plus a faithful simulated implementation.…, Everything a runner needs to execute one deployment., One streamed output line for a step., Stream output lines for *step_name*; raise StepFailure to fail it., RunContext, _slug() (+1 more)
...
### Community 16 - "AuthContext"
Cohesion: 0.06
Nodes (85): AuthContext, Resolved identity attached to every authenticated request., deployment_log_channel(), Deployment, DeploymentStep, AlertSeverity, DeploymentStatus, StepStatus (+77 more)
...
Cohesion: 0.08
Nodes (44): list_audit_logs(), datetime, DbSession, Depends, get, PageParamsDep, Audit log API. Strictly read-only: the table is append-only by design., Newest-first audit trail with optional filters. No write routes exist for this… (+36 more)
...
Cohesion: 0.12
Nodes (22): create_access_token(), password_needs_rehash(), UUID, Return ``(token, jti)`` for a short-lived access token., tokens_equal(), verify_password(), Unit tests for app.core.security — no database or Redis required., The Secure-cookie decision follows the wire, not the environment label.… (+14 more)
...
Cohesion: 0.09
Nodes (23): 1. Scope and stance, 2.1 Current shape (real), 2.2 Changes for the target model, 2.3 Capabilities (target), 2.4 DockerEndpoint — the 0..1 relation, 2. The Node concept, 5.1 Model and lifecycle, 5.2 Whitelisted operation types (+15 more)
...
Cohesion: 0.06
Nodes (36): AgentClient, build_heartbeat(), collect_container_stats(), collect_containers(), cpu_percent_since(), disk_stats(), _docker_request(), load1() (+28 more)
...
Cohesion: 0.19
Nodes (15): _as_health(), _as_status(), _daily_dedup(), _dispatch(), _emit(), _mark_host_unavailable(), Any, AsyncSession (+7 more)
...
Cohesion: 0.08
Nodes (50): NotFound, create_api_key(), AsyncSession, Request, UUID, Revoke a key owned by *owner_id*. Foreign keys look like NotFound (no leak)., Revoke every live key of a user (used on deactivation). Returns count., Validate + normalise scopes; returns the deduplicated list. Accepts exact… (+42 more)

### Community 36 - "monitors.py"
Cohesion: 0.12
Nodes (40): check_now(), create_monitor(), delete_monitor(), get_monitor(), list_checks(), list_monitor_incidents(), list_monitors(), pause_monitor() (+32 more)
...
Cohesion: 0.06
Nodes (67): agent_heartbeat(), agent_hello(), DbDep, post, Request, Response, Resolve ``X-Agent-Token`` to an enrolled server or raise 401., First contact after enrollment: persist static host facts, negotiate cadence. (+59 more)
...
Cohesion: 0.16
Nodes (18): ContainerHealth, Agent-facing schemas. Payloads are data only — never executed., agent_fixture(), _load_agent(), _patch_stats(), Any, fixture, MonkeyPatch (+10 more)
...
Cohesion: 0.08
Nodes (58): BadRequest, UnprocessableEntity, assert_safe_tcp_endpoint(), assert_safe_url(), _is_forbidden_address(), is_simulation_url(), _raise_block(), SSRF guard applied to every operator-supplied outbound URL. Monitors and… (+50 more)

### Community 49 - "secret_service.py"
Cohesion: 0.18
Nodes (24): An encrypted configuration value. The plaintext is never returned by the API…, Secret, _actor_type(), create_secret(), delete_secret(), _detail(), get_secret(), get_secret_detail() (+16 more)
...
Cohesion: 0.07
Nodes (36): create_api_key(), list_api_keys(), CurrentUser, DbSessionDep, delete, get, post, Request (+28 more)
...
Cohesion: 0.08
Nodes (26): assert_safe_url_async(), Async wrapper around :func:`assert_safe_url`; DNS runs off the loop., CheckOutcome, coerce_headers(), _decode(), get_transport(), MonitorTransport, Any (+18 more)

### Community 56 - "require_permission"
Cohesion: 0.14
Nodes (27): permission_dep(), Any, Dependency factory enforcing a permission; returns 403 when denied., require_permission(), _check(), create_secret(), delete_secret(), get_secret() (+19 more)
...
Cohesion: 0.13
Nodes (27): assert_error_code(), cookie_attributes(), login_account(), Response, Assert envelope shape + code; returns the inner error object., The raw Set-Cookie header carrying the refresh token., Extract just the opaque token from a Set-Cookie header., Parse Set-Cookie attributes (lowercased keys, empty string for flags). (+19 more)
...
Cohesion: 0.12
Nodes (41): CredentialKind, EventLevel, IncidentEventKind, IncidentSeverity, IncidentStatus, LogSource, MetricGranularity, Closed registries of domain enumerations. Member names equal their values… (+33 more)
...
Cohesion: 0.13
Nodes (16): digest_of(), Short server-verifiable digest used for change detection display. HMAC-SHA256…, test_digest_is_keyed_not_plain_sha256(), test_digest_length_and_determinism(), 10. Status/expiry tracking and monitor tie-in, 12. Audit and events, 13. Non-goals, 2. Certificate entity (+8 more)
...
Cohesion: 0.14
Nodes (36): NotificationChannel, A delivery target (email address / webhook endpoint). ``config_ciphertext``…, _as_uuid(), _audit(), create_channel(), decode_delivery_cursor(), decrypt_channel_config(), delete_channel() (+28 more)
...
Cohesion: 0.11
Nodes (25): async_sessionmaker, AsyncEngine, _decode_cursor(), _encode_cursor(), list_event_types(), list_events(), datetime, DbSession (+17 more)
...
Cohesion: 0.21
Nodes (13): get_current_user(), get_optional_user(), _load_permissions(), AsyncSession, DbSessionDep, Request, Standard authentication dependency. Also records client IP for logs/audit., Like :func:`get_current_user` but returns ``None`` for anonymous callers. (+5 more)
...
Cohesion: 0.20
Nodes (17): SystemEvent, delete_host(), Hard-delete a host; child containers/images cascade via FK., _after_commit_publish(), _after_rollback_drop(), event_frame_from_row(), publish(), _publish_after_commit() (+9 more)
...
Cohesion: 0.19
Nodes (13): get_settings(), Return the cached settings singleton., configure_logging(), _orjson_dumps(), Any, Configure structlog + stdlib logging once at process start. Everything…, decode_access_token(), Any (+5 more)

### Community 90 - "channel.py"
Cohesion: 0.18
Nodes (14): ChannelType, DeliveryStatus, ChannelBase, ChannelCreate, ChannelUpdate, DeliveryOut, EmailConfig, BaseModel (+6 more)

### Community 91 - "Deployment Architecture — NexusOps (target state)"
Cohesion: 0.10
Nodes (21): Exception, Raised by a runner when a step fails irrecoverably., StepFailure, 10. Events and WS during runs, 11. Post-deploy hooks, 12. Rollback semantics (existing behavior kept), 13. The simulated runner stays — labeled, 14. Open questions (+13 more)
...
Cohesion: 0.12
Nodes (17): 10. Resolution flow at deploy time, 1.1 Storage and crypto, 1.2 Data model, 1.3 API surface (metadata-only reads), 1.5 SILENT DEGRADE BUG (must-fix), 1.6 Current-state gap list, 1. Current state, 2.1 Resolution runs inside org scope (+9 more)

### Community 102 - "encrypt_str"
Cohesion: 0.13
Nodes (21): decrypt_str(), encrypt_str(), _fernet(), _collect_refs(), Recursively collect ``${secret:KEY}`` references from config values., Resolve every ``${secret:KEY}`` reference in an environment's config. INTERNAL…, resolve_secrets_for_environment(), test_notification_pipeline_queues_delivery_for_down_event() (+13 more)
...
Cohesion: 0.14
Nodes (19): clip(), DockerProviderError, Exception, Provider abstraction for docker hosts (real daemon or simulated). Providers are…, A provider operation failed. ``str()`` is safe to show/log., Build a compact, secret-free description of a provider failure. Only the…, Truncate *text* to *limit* characters, stripping control chars., sanitize_error() (+11 more)
...
Cohesion: 0.08
Nodes (25): LogLine, One parsed log record from a container log stream., datetime, Replay registered LogEntry rows ordered by ts (tail N). ``follow`` is ignored…, collect_recent_logs(), Pull recent logs from running containers of a real host. Simulated hosts are…, append_lines(), count_container_logs() (+17 more)
...
### Community 155 - "test_rbac.py"
Cohesion: 0.22
Nodes (12): bearer(), A fresh, deliverable-shaped address unique to a single test. ``example.com`` is…, unique_email(), Sessions must carry the real client address, not the proxy hop's peer. Behind…, Client-injected leftmost XFF entries must not poison the session IP., test_login_ignores_spoofed_forwarded_entries(), test_login_records_forwarded_client_ip_in_session(), _invite_user() (+4 more)
...
### Community 165 - "3. Agent lifecycle"
Cohesion: 0.33
...
- **Why does `AuthContext` connect `AuthContext` to `APIModel`, `deps.py`, `project_service.py`, `incidents.py`, `PageParams`, `containers.py`, `get_latest_server_metrics`, `monitor_service.py`, `auth_service.py`, `DockerHost`, `NotFound`, `monitors.py`, `server_service.py`, `v1/channels.py`, `v1/deployments.py`, `test_permissions.py`, `servers.py`, `secret_service.py`, `role_service.py`, `main.py`, `require_permission`, `enums.py`, `middleware.py`, `notification_service.py`, `Hub`, `events.py`, `docker_hosts.py`, `scope_matches`, `resolve_auth`, `publish`, `create_host`, `DockerProviderError`?**
